...
## 使用 Foxglove 建图和查看摄像头

`mapping.launch.py` 默认不启动 RViz，而是启动 Foxglove Bridge 和 USB 摄像头节点：
...
在上位机 Foxglove 中添加 WebSocket 连接：
...
- `3D`：显示 `/map`、`/scan` 和 TF
- `Image`：选择 `/camera/image/compressed`
- `Raw Messages`：检查 `/odom`、`/scan` 或 `/map`

摄像头默认使用 `/dev/video0`，分辨率为 `640x480`，帧率为 `15 FPS`，以 JPEG 压缩图像发布。可以在启动时覆盖参数：

```bash
ros2 launch lidar_py_pkg mapping.launch.py \
  camera_device:=/dev/video0 \
  camera_width:=640 camera_height:=480 camera_framerate:=15.0
```

临时关闭摄像头或 Foxglove：

```bash
ros2 launch lidar_py_pkg mapping.launch.py use_camera:=false
ros2 launch lidar_py_pkg mapping.launch.py use_foxglove:=false
```

如需临时恢复 RViz：

```bash
ros2 launch lidar_py_pkg mapping.launch.py use_rviz:=true use_foxglove:=false
```
