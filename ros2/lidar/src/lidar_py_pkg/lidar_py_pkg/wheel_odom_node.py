...
class WheelOdomNode(Node):
    def __init__(self):
        ...
        self.cmd_vel_rate = float(self.get_parameter("cmd_vel_rate").value)
        ...
        self.cmd_timer = self.create_timer(1.0 / max(self.cmd_vel_rate, 1.0), self.send_cmd_vel)
    ...
    def cmd_vel_callback(self, message):
        self.last_cmd = (float(message.linear.x), float(message.angular.z))
        ...
        if time.monotonic() - self.last_cmd_time > self.cmd_vel_timeout:
            if not self.stop_sent:
                try:
                    ...
                    pass
            return
        linear_m_s, angular_rad_s = self.last_cmd
        linear_mm_s = max(-100.0, min(100.0, linear_m_s * 1000.0))
        angular_deg_s = max(-120.0, min(120.0, math.degrees(angular_rad_s)))
        command = f"NAV {linear_mm_s:.3f} {angular_deg_s:.3f}\n".encode("ascii")
        try:
            self.serial.write(command)
        except serial.SerialException as exc:
            self.get_logger().warning(f"cmd_vel serial write failed: {exc}")
